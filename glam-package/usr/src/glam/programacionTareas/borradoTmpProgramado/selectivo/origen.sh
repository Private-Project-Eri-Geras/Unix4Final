...
while true; do
    origen=$(dialog --title "Selecciona un temporal" \
        --cancel-label "Cancelar" \
        --help-button --help-label "Ayuda" \
        --stdout --cursor-off-label --fselect /tmp/ 14 70)
    opcion=$?
    ...
    # Si el usuario presiona "Slect"
        ...
        if [ -f "$origen" ]; then
            dialog --title "CONFIRMAR" --yesno "¿Deseas usar el temporal $origen?" 0 0
            opcion=$?
            if [ $opcion -eq 0 ]; then
                echo -n "rm -f $origen" >/tmp/origen.tmp
                numeroFinSelectivo=$(grep -n "# FIN TMP SELECTIVO" /etc/crontab | cut -d ':' -f 1)
                head -$((numeroFinSelectivo - 1)) /etc/crontab >/tmp/crontab
            ...
            opcion=$?
            if [ $opcion -eq 0 ]; then
                echo -n "rm -rf $origen" >/tmp/origen.tmp
                numeroFinSelectivo=$(grep -n "# FIN TMP SELECTIVO" /etc/crontab | cut -d ':' -f 1)
                head -$((numeroFinSelectivo - 1)) /etc/crontab >/tmp/crontab
                cat /tmp/cron.tmp /tmp/origen.tmp >>/tmp/crontab
                echo "" >>/tmp/crontab
