...
mostrar_ayuda() {
    dialog --title "Help" --msgbox \
        "\n\
    Use Espacio para seleccionar las programaciones y pulse OK cuando haya terminado" 0 0
}
...
while true; do
    OPTIONS=$(dialog --no-tags --separate-output --clear --title "Selección de programaciones" \
            --checklist "Use Espacio para seleccionar los respaldos y pulse OK cuando haya terminado." 0 0 0 \
        # INICIO TMP SELECTIVO
        # FIN TMP SELECTIVO
        2>&1 >/dev/tty) 
    ...
    dialog --title "Confirmación" --yesno "¿Está seguro que desea eliminar $eliminaciones programaciones?" 0 0
    if [ $? -eq 1 ]; then
        clear
    ...
    # re escribir crontab
    head -$((numeroInicioSelectivo)) /etc/crontab >/tmp/crontab
    cat /tmp/list.tmp >>/tmp/crontab
    tail -n +$((numeroFinSelectivo)) /etc/crontab >>/tmp/crontab
    rm -f /tmp/list.tmp
