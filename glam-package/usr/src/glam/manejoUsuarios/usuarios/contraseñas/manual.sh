...
mostrar_ayuda() {
    ...
    dialog --backtitle "CAMBIO CONTRASEÑAS MANUAL" --title "AYUDA" \
        --exit-label "Ok" \
...
chPasswd() {
    ...
    touch /var/glam/tmp/temp_passwdForm
    dialog --title "CAMBIAR CONTRASÑA" --form "\nDialog Sample Label and Values" 0 50 2 \
        ...
        2>/var/glam/tmp/temp_passwdForm
    form_exit_code=$?
    if [[ $form_exit_code -eq 1 ]]; then
        return
    fi
    # Linea 1 del archivo
    newPasswd=$(head -n 1 /var/glam/tmp/temp_passwdForm)
    # Linea 2 del archivo
    confirmPasswd=$(tail -n 1 /var/glam/tmp/temp_passwdForm)
    rm /var/glam/tmp/temp_passwdForm
...
update_dialog() {
    ...
    dialog --no-clear --no-hot-list --colors --title "CAMBIAR CONTRASEÑA" --begin "$input_y" "$input_x" --infobox "$(cat /var/glam/tmp/dialog_content)" "$input_height" "$input_width" \
        --and-widget \
